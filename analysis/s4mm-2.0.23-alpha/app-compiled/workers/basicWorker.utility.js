"use strict";
const { parentPort } = require('node:worker_threads') ? require('electron') : { parentPort: null };
if (!parentPort) {
    // Wenn das oben nicht geht, nimm den direkten Prozess-Zugriff
    // In UtilityProzessen injiziert Electron das oft direkt
    var activePort = process.parentPort || require('electron').parentPort;
}
else {
    var activePort = parentPort;
}
if (!activePort) {
    console.error("Worker konnte keinen parentPort finden!");
}
else {
    activePort.on('message', (e) => {
        const data = e.data || e; // Je nach Version ist es ein MessageEvent oder direkt Daten
        console.log("Worker empfing:", data);
        // Deine Case-Logik...
        activePort.postMessage({ result: "Done", index: data.index });
    });
}
