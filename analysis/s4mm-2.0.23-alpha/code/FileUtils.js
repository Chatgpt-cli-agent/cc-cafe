"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
exports.FileUtils = void 0;
var shell = require('electron').shell;
var isMac = process.platform === "darwin";
var winTrash = require('win-trash');
var recyclebin = require('recycle-bin');
var fs = require('fs');
var FileUtils = /** @class */ (function () {
    function FileUtils() {
    }
    FileUtils.deleteFile = function (filepath, trash) {
        return __awaiter(this, void 0, void 0, function () {
            var firstTry, secondTry, thirdTry, fourthTry, fifthTry, error_1, error_2, error_3, error_4, error_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        firstTry = false;
                        secondTry = false;
                        thirdTry = false;
                        fourthTry = false;
                        fifthTry = false;
                        console.log("[DELETE-INFO] Try to delete:  - " + filepath);
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        return [4 /*yield*/, shell.trashItem(filepath)];
                    case 2:
                        _a.sent();
                        firstTry = true;
                        return [3 /*break*/, 4];
                    case 3:
                        error_1 = _a.sent();
                        console.log("[DELETE-FAILED] NATIV DELETE FAILED  - " + filepath);
                        console.log(error_1);
                        return [3 /*break*/, 4];
                    case 4:
                        if (firstTry && !fs.existsSync(filepath))
                            return [2 /*return*/, true];
                        _a.label = 5;
                    case 5:
                        _a.trys.push([5, 7, , 8]);
                        return [4 /*yield*/, trash["default"](filepath)];
                    case 6:
                        _a.sent();
                        secondTry = true;
                        return [3 /*break*/, 8];
                    case 7:
                        error_2 = _a.sent();
                        console.log("[DELETE-FAILED] NPM TRASH  - " + filepath);
                        console.log(error_2);
                        return [3 /*break*/, 8];
                    case 8:
                        if (secondTry && !fs.existsSync(filepath))
                            return [2 /*return*/];
                        _a.label = 9;
                    case 9:
                        _a.trys.push([9, 12, , 13]);
                        if (!!isMac) return [3 /*break*/, 11];
                        return [4 /*yield*/, winTrash([filepath])];
                    case 10:
                        _a.sent();
                        thirdTry = true;
                        _a.label = 11;
                    case 11: return [3 /*break*/, 13];
                    case 12:
                        error_3 = _a.sent();
                        console.log("[DELETE-FAILED] OTHER SHELL  - " + filepath);
                        console.log(error_3);
                        return [3 /*break*/, 13];
                    case 13:
                        if (thirdTry && !fs.existsSync(filepath))
                            return [2 /*return*/];
                        _a.label = 14;
                    case 14:
                        _a.trys.push([14, 16, , 17]);
                        return [4 /*yield*/, trash["default"]([filepath])];
                    case 15:
                        _a.sent();
                        fourthTry = true;
                        return [3 /*break*/, 17];
                    case 16:
                        error_4 = _a.sent();
                        console.log("[DELETE-FAILED] TRASH ARRAY  - " + filepath);
                        console.log(error_4);
                        return [3 /*break*/, 17];
                    case 17:
                        if (fourthTry && !fs.existsSync(filepath))
                            return [2 /*return*/];
                        _a.label = 18;
                    case 18:
                        _a.trys.push([18, 20, , 21]);
                        return [4 /*yield*/, recyclebin([filepath])];
                    case 19:
                        _a.sent();
                        fifthTry = true;
                        return [3 /*break*/, 21];
                    case 20:
                        error_5 = _a.sent();
                        console.log("[DELETE-FAILED] recyclebin  - " + filepath);
                        console.log(error_5);
                        return [3 /*break*/, 21];
                    case 21:
                        if (fifthTry && !fs.existsSync(filepath))
                            return [2 /*return*/];
                        throw new Error('[DELETE-FAILED] ALL WAYS');
                }
            });
        });
    };
    FileUtils.deleteAllByIno = function (knex, ino) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, knex.from('CasParts').del().where('ino', ino)];
                    case 1:
                        _a.sent();
                        return [4 /*yield*/, knex.from('Casp').del().where('ino', ino)];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, knex.from('Cobj').del().where('ino', ino)];
                    case 3:
                        _a.sent();
                        return [4 /*yield*/, knex.from('Entries').del().where('ino', ino)];
                    case 4:
                        _a.sent();
                        return [4 /*yield*/, knex.from('Files').del().where('ino', ino)];
                    case 5:
                        _a.sent();
                        return [4 /*yield*/, knex.from('Thumbnails').del().where('ino', ino)];
                    case 6:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    return FileUtils;
}());
exports.FileUtils = FileUtils;
